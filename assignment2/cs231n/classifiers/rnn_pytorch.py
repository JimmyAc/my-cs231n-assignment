...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # 输入: features (N, D) -> 输出: h0 (N, H)
        # 这一步将图像的视觉信息注入到 RNN 的初始状态中。
        h0 = affine_forward(features, W_proj, b_proj)

        # (2) 使用词嵌入层将 captions_in 中的单词索引转换为向量
        # 输入: captions_in (N, T) -> 输出: x (N, T, W)
        x = word_embedding_forward(captions_in, W_embed)

        # (3) 使用 vanilla RNN 或 LSTM 处理输入词向量序列
        # 输入: x (N, T, W), h0 (N, H) -> 输出: h (N, T, H)
        ...
        # (4) 使用(时间)仿射变换计算每个时间步的词汇表得分
        # 将隐藏状态维度 H 映射回词汇表大小 V
        # 输入: h (N, T, H) -> 输出: scores (N, T, V)
        scores = temporal_affine_forward(h, W_vocab, b_vocab)

        # (5) 使用(时间) softmax 计算 loss
        # 对比预测得分 scores 和真实标签 captions_out，并应用 mask 忽略 <NULL> 标记
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        # 1. 初始化隐藏状态 (使用 affine_forward)
        # 将图像特征映射到隐藏状态空间: (N, D) -> (N, H)
        h = affine_forward(features, W_proj, b_proj)
        
        # 如果是 LSTM，还需要初始化细胞状态 c 为全零
        ...
        for t in range(max_length):
            # (1) 词嵌入 (使用 word_embedding_forward)
            # 输入 current_word 形状 (N,) -> 输出 word_embed 形状 (N, W)
            # 这里的 helper 函数 W[x] 支持直接对 (N,) 形状的输入进行查表
            word_embed = word_embedding_forward(current_word, W_embed)

            # (2) RNN/LSTM 步进 (使用 rnn_step_forward 或 lstm_step_forward)
            if self.cell_type == 'rnn':
                # 输入: word_embed (N, W), 上一步 h (N, H) -> 输出: 新 h (N, H)
                h = rnn_step_forward(word_embed, h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                # 输入: word_embed, 上一步 h, 上一步 c -> 输出: 新 h, 新 c
                h, c = lstm_step_forward(word_embed, h, c, Wx, Wh, b)
            
            # (3) 计算词汇表得分 (使用 affine_forward)
            # 注意：这里是对单步 hidden state 做投影，所以用普通的 affine_forward
            # 输入: h (N, H) -> 输出: scores (N, V)
            scores = affine_forward(h, W_vocab, b_vocab)

            # (4) 选择得分最高的词
            # scores 形状 (N, V)，在 dim=1 上取 max
